...
public class Data extends Activity {
    ...
    public void createTable(){
        try {
            String sql = "create table " + tableName + "(id integer primary key autoincrement, name TEXT not null unique, lon TEXT, lat TEXT, type TEXT, tel TEXT)";
            db.execSQL(sql);
    ...
    public void insertData(String name,Double lon,Double lat,String type,String tel) {
        try {
            String sql = "INSERT INTO " + tableName + " VALUES (NULL, '" + name + "', '" + lon + "', '" + lat + "', '" + type + "', '" + tel + "'); + ON DUPLICATE KEY UPDATE name = " + name + ";)";
            db.execSQL(sql);
        }catch(Exception e){
